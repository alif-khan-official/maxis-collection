...
class ManageCollectionComponent extends React.Component {
	...
	putAway() {
		...
		try {
			fetch(gwUrl + "collection-service/endpoint/collection/put-away", {
				method: "POST",
				headers: {
					"Content-Type": "application/json",
					"token": "Bearer " + AuthUtil.getIdToken()
	...
	downloadPDFMini() {
		...
		for (let j = 0; j < this.state.data.length; j++) {
			...
			row.push(this.state.data[j].feeDetails.merchantEncashmentFeeAmount);
			row.push(this.state.data[j].feeDetails.merchantEncashmentReceiveAmount);
		...
		console.log(pdfTable);
		let title = "Collection Report (" + AuthUtil.getName() + ")";
	...
	downloadPDF() {
		...
		for (let j = 0; j < this.state.data.length; j++) {
			...
			row.push(this.state.data[j].payeeId);
			row.push(this.state.data[j].payeeName);
			row.push(this.state.data[j].payeeWallet != null?this.state.data[j].payeeWallet.walletId:"");

			row.push(this.state.data[j].cashPointId);
			row.push(this.state.data[j].cpName);
			row.push(this.state.data[j].cashpointWallet != null?this.state.data[j].cashpointWallet.walletId:"");
	...
	prepareDownload(source) {
		...
		if (roleId === Constant.ROLE_NAME_MERCHANT)
			for (let j = 0; j < currentRecords.length; j++) {
				...
				dataArray[18] = currentRecords[j].feeDetails ? currentRecords[j].feeDetails.merchantEncashmentFeeAmount : "";
				dataArray[19] = currentRecords[j].feeDetails ? currentRecords[j].feeDetails.merchantEncashmentReceiveAmount : "";
		...
		else if (roleId === Constant.ROLE_NAME_CASHPOINT)
			for (let j = 0; j < currentRecords.length; j++) {
				let dataArray = [];
				...
				dataArray[9] = currentRecords[j].collectionTypeName;
				dataArray[10] = currentRecords[j].invoiceNo;
				dataArray[11] = currentRecords[j].reference1;

				dataArray[12] = currentRecords[j].reference2;
				dataArray[13] = currentRecords[j].reference3;
	...
	getColumns() {
		...
		let roleList = AuthUtil.getRoleList();
		let roleId = roleList != null && roleList !== undefined && roleList[0] ? roleList[0].name : "";
		...
		let columnsMerchant = [
			...
			},
			{
				Header: "Fee",
				minWidth: 150,
				accessor: "feeDetails.merchantEncashmentFeeAmount"
			},
			{
				Header: "Receivable",
				minWidth: 150,
				accessor: "feeDetails.merchantEncashmentReceiveAmount"
			}
		];
...
export default withRouter(ManageCollectionComponent);
